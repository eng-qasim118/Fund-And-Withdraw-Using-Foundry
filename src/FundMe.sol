...
contract FundMe {
    ...
    constructor(address _aggregator ) {
        i_owner = msg.sender;
        aggregatorV3 = AggregatorV3Interface(_aggregator);
    }
    ...
    function withdrawCheaper() public onlyOwner {
        uint arrayLength = s_funders.length;
        for (uint256 funderIndex = 0; funderIndex < arrayLength; funderIndex++) {
            address funder = s_funders[funderIndex];
            s_addressToAmountFunded[funder] = 0;
        }
        s_funders = new address[](0);
    ...
    function getAddressToAmmount(address _address) public view returns (uint){
